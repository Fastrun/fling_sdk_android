...
final class CastDeviceManager_axb implements CastSocketListener_atu {
    final CastSocket_att mCastSocket_a;
    final CastDevice mCastDevice_b;
    boolean mNoApp_c;
    boolean mNoNamespace_d;
    boolean e;
    final DeviceFilter_awz mDeviceFilter_f;
    private final String g;
    private final AppInfoHelper_axa h = new AppInfoHelper_axa((byte) 0);
    private final JSONArray i = new JSONArray();

    public CastDeviceManager_axb(DeviceFilter_awz deviceFilter, CastDevice castDevice)
    {
        super();
        ...
        e = true;
        mCastSocket_a = new CastSocket_att(DeviceFilter_awz.getContext_a(deviceFilter), this);
        mCastDevice_b = castDevice;
        Object aobj[] = new Object[2];
        aobj[0] = DeviceFilter_awz.b(deviceFilter);
        aobj[1] = Long.valueOf(DeviceFilter_awz.a().incrementAndGet());
        g = String.format("%s-%d", aobj);
        if (DeviceFilter_awz.getDiscoveryCriterias_c(deviceFilter).size() > 0)
        {
            Iterator iterator = DeviceFilter_awz.getDiscoveryCriterias_c(deviceFilter).iterator();
    ...
    private void updateStatus_d()
    {
        ...
        {
            Iterator iterator;
            DiscoveryCriteria_aty aty1;
            AppInfoHelper_axa axa1;
            String s;
            try
            {
                ...
                DeviceFilter_awz.getLogs_b().d(ioexception, "Failed to send disconnect message",
                        new Object[0]);
            } catch (JSONException jsonexception) {
                DeviceFilter_awz.getLogs_b().e(jsonexception, "Failed to build disconnect message",
                        new Object[0]);
            } catch (Exception e) {
                e.printStackTrace();
                DeviceFilter_awz.getLogs_b().d(e, "Failed to send disconnect message",
                        new Object[0]);
            }
            mCastSocket_a.disconnect_b();
        }
        hashset = new HashSet();
        Iterator iterator = DeviceFilter_awz.getDiscoveryCriterias_c(mDeviceFilter_f).iterator();
        do
        {
            if (!iterator.hasNext())
                break;
    ...
    public final void onConnected_a()
    {
        try
        {
            sendMessage_a(
                    ...
                    (new JSONObject()).put("type", "CONNECT")
                            .put("package", DeviceFilter_awz.b(mDeviceFilter_f))
        ...
        {
            DeviceFilter_awz.getLogs_b()
                    .e(jsonexception, "Failed to build messages", new Object[0]);
        } catch (Exception e) {
            DeviceFilter_awz.getLogs_b().e(e, "Failed to send messages", new Object[0]);
            e.printStackTrace();
        }
    }
    ...
    public final void onConnectionFailed_a(int j)
    {
        ...
        DeviceFilter_awz.getHandler_d(mDeviceFilter_f).post(new Runnable() {
    ...
    public final void onMessageReceived_a(ByteBuffer bytebuffer)
    {
        ...
        try
        {
            ...
            Logs_avu avu3 = DeviceFilter_awz.getLogs_b();
        ...
        {
            Logs_avu avu4 = DeviceFilter_awz.getLogs_b();
    ...
    {
        DeviceFilter_awz.getLogs_b().d("Device filter disconnected", new Object[0]);
    }
}
