...
public final class CastSocketMultiplexer_atv {
    private static Logs_avu mLogs_a = new Logs_avu("CastSocketMultiplexer");
    private static CastSocketMultiplexer_atv mInstance_b;
    ...
    private CastSocketMultiplexer_atv(Context context)
    {
        mContext_k = context;
    }

    public static CastSocketMultiplexer_atv getInstance_a(Context context)
    {
        if (mInstance_b == null) {
            mInstance_b = new CastSocketMultiplexer_atv(context);
        }
    ...
    static CountDownLatch getCountDownLatch_a(CastSocketMultiplexer_atv atv1)
    {
        return atv1.mCountDownLatch_m;
    }

    static synchronized void processData_b(CastSocketMultiplexer_atv multiplexer)
    {
        ...
        while (!multiplexer.mQuitLoop_f) {
            ...
            if (multiplexer.mReadyToConnect_i.getAndSet(false)) {
                synchronized (multiplexer.mAllCastSocketList_d) {
                    ...
                    while (iterator3.hasNext()) {
                        CastSocket_att castSocket = (CastSocket_att) iterator3.next();
                        try {
            ...
            {
                for (Iterator iterator2 = errorCastSocketList.iterator(); iterator2.hasNext(); ((CastSocket_att) iterator2
            ...
            synchronized (multiplexer.mRegistedCastSocketList_c) {
                ...
                {
                    CastSocket_att castSocket = (CastSocket_att) iterator.next();
            ...
            if (i1 != 0) {
                ...
                while (iterator1.hasNext())
                {
                    try
                    {
                        ...
                        CastSocket_att castSocket = (CastSocket_att) selectionkey.attachment();
    ...
    static Thread d(CastSocketMultiplexer_atv atv1)
    {
            ...
            mCastSocketMultiplexerThread_h = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try
                    {
                        ...
                        processData_b(CastSocketMultiplexer_atv.this);
                        return;
                    } catch (Throwable localThrowable)
                    {
                        CastSocketMultiplexer_atv.c().e(localThrowable,
    ...
    public final synchronized void doConnect_a(CastSocket_att castSocket)
    {
        try {
            checkStatus_d();
