...
import com.fireflycast.client.internal.RequestTracker;
...
public class MediaControlChannelImpl extends MediaControlChannel {
	...
	private final RequestTracker mRequestTrackerLoad_yo = new RequestTracker(
			yj);
	private final RequestTracker mRequestTrackerPause_yp = new RequestTracker(
			yi);
	private final RequestTracker mRequestTrackerPlay_yq = new RequestTracker(
			yi);
	private final RequestTracker mRequestTrackerStop_yr = new RequestTracker(
			yi);
	private final RequestTracker mRequestTrackerSeed_ys = new RequestTracker(
			yk);
	private final RequestTracker mRequestTrackerVolume_yt = new RequestTracker(
			yi);
	private final RequestTracker mRequestTrackerMute_yu = new RequestTracker(
			yi);
	private final RequestTracker mRequestTrackerRequestStatus_yv = new RequestTracker(
			yi);
	...
	public long load_a(RequestTrackerCallback paramdx, MediaInfo mediaInfo, boolean autoplay,
		...
		try {
			...
			json.put("currentTime",
					DoubleAndLongConverter.long2double_l(currentTime));
			if (customData != null) {
	...
	public long pause_a(RequestTrackerCallback paramdx, JSONObject customData) throws IOException {
		JSONObject json = new JSONObject();
		long requestId = getRequestId_cW();
	...
	public long stop_b(RequestTrackerCallback paramdx, JSONObject customData) throws IOException {
		JSONObject json = new JSONObject();
		long requestId = getRequestId_cW();
	...
	public long play_c(RequestTrackerCallback paramdx, JSONObject customData)
	...
	public long seed_a(RequestTrackerCallback paramdx, long currentTime, int resumeState,
	...
	public long setStreamVolume_a(RequestTrackerCallback paramdx, double level,
			JSONObject customData) throws IOException,
	...
	public long setStreamMute_a(RequestTrackerCallback paramdx, boolean muted, JSONObject customData)
			throws IOException, IllegalStateException {
		JSONObject json = new JSONObject();
		long requestId = getRequestId_cW();
	...
	public long requestStatus_a(RequestTrackerCallback paramdx) throws IOException {
		JSONObject json = new JSONObject();
		long requestId = getRequestId_cW();
	...
	private class RequestTrackerTask_a implements Runnable {
		public void run() {
			...
			boolean bool = false;
			synchronized (RequestTracker.mLock_yD) {
