...
public abstract class FireflyClient<T extends IInterface> implements
		FireflyPlayServicesClient, Api.ConnectionApi_a,
		FireflyClientEvents.FmsClientEventCallback_b {
	...
	private final FireflyClientEvents mFmsClientEvent_zx;

	protected FireflyClient(Context context, Looper looper,
			FireflyApiClient.ConnectionCallbacks callbacks,
			FireflyApiClient.OnConnectionFailedListener failedListener,
			String[] strArray) {
		ValueChecker.checkNullPointer_f(context);
		mContext = context;
		ValueChecker.checkNullPointer_b(looper, "Looper must not be null");
		mLooper_zs = looper;
		mFmsClientEvent_zx = new FireflyClientEvents(context, looper, this);
		mFireflyClientHandler = new FireflyClientHandler_a(looper);

		ValueChecker.checkNullPointer_f(callbacks);
		registerConnectionCallbacks(callbacks);

		ValueChecker.checkNullPointer_f(failedListener);
		registerConnectionFailedListener(failedListener);
	}

	protected FireflyClient(
	...
	public void connect() {
		...
		if (mFmsClientServiceConnection_Bx != null) {
			...
			mService_Bv = null;
			FireflyServiceManager.getInstance_y(mContext).unbindService_b(
					getServiceName_aF(), mFmsClientServiceConnection_Bx);
		}
		mFmsClientServiceConnection_Bx = new FireflyClientServiceConnection_f();
		boolean bool = FireflyServiceManager.getInstance_y(mContext)
	...
	public void disconnect() {
		...
		if (this.mFmsClientServiceConnection_Bx != null) {
			FireflyServiceManager.getInstance_y(this.mContext)
	...
	public static final class IFireflyCallbackImpl_e extends
			IFireflyCallbacks.Stub_a {
		private FireflyClient castClient_BF;

		public IFireflyCallbackImpl_e(FireflyClient client) {
	...
	public final class FireflyClientCallbackProxy_h extends
		...
		protected void onCallback_a(Boolean paramTListener) {
			...
			case 0:
				...
				FireflyServiceManager.getInstance_y(mContext)
						.unbindService_b(getServiceName_aF(),
								mFmsClientServiceConnection_Bx);
			...
			default:
				...
				if (mFmsClientServiceConnection_Bx != null) {
					FireflyServiceManager.getInstance_y(mContext)
							.unbindService_b(
									FireflyClient.this.getServiceName_aF(),
									mFmsClientServiceConnection_Bx);
					mFmsClientServiceConnection_Bx = null;
				}
				mConnectedState_By = CONNECTION_STATUS_DISCONNECTED;
	...
	protected final void getServiceFromBroker_x(IBinder paramIBinder) {
		try {
			IFireflyServiceBroker serviceBroker = IFireflyServiceBroker.Stub_a
